...
from ..schemas.movimento import DashboardResposta, LinhaAtingimento, BreakdownProdutosResposta, LinhaProduto
...
def breakdown_produtos(
    ano: int,
    periodo_tipo: str = Query("mensal"),
    periodo_ref: int = Query(...),
    ...
    if u.perfil == "vendedor":
        raise HTTPException(status_code=403, detail="Acesso negado para vendedor")
    ...
    meta_stmt = _filtros(meta_stmt, Meta)
    metas_por_produto = {p: (v or Decimal(0)) for p, v in db.execute(meta_stmt).all()}

    real_stmt = (
        select(Realizado.produto_id, func.sum(Realizado.valor))
        .where(
            Realizado.ativo.is_(True),
            extract("year", Realizado.data_venda) == ano,
            extract("month", Realizado.data_venda).in_(meses),
        )
    ...
    real_stmt = _filtros(real_stmt, Realizado)
    reals_por_produto = {p: (v or Decimal(0)) for p, v in db.execute(real_stmt).all()}

    prod_ids = set(metas_por_produto) | set(reals_por_produto)
    prod_nomes = {p: n for p, n in db.execute(
        select(Produto.id, Produto.nome).where(Produto.id.in_(prod_ids or {-1}))
    ).all()}

    produtos = []
    for pid in sorted(prod_ids, key=lambda p: prod_nomes.get(p, "")):
        m = metas_por_produto.get(pid, Decimal(0))
        r = reals_por_produto.get(pid, Decimal(0))
        produtos.append(LinhaProduto(
            produto_id=pid,
            produto_nome=prod_nomes.get(pid),
            meta=m,
            realizado=r,
            percentual=_pct(m, r),
        ))

    return BreakdownProdutosResposta(
        ano=ano,
